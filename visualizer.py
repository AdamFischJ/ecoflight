...
def plot_wind_pattern(wind_data, output_path="outputs/wind_pattern.png"):
    ...
    ax1.plot(wind_data['timestamp'], wind_data['wind_direction_deg'], color='tab:blue', marker='o', label='Direction')
    ax1.tick_params(axis='y', labelcolor='tab:blue')
    ax1.set_xticks(wind_data['timestamp'][::3])
    ax1.set_xticklabels(wind_data['timestamp'][::3], rotation=45, ha='right')

    ax2 = ax1.twinx()
    ax2.set_ylabel("Wind Speed (m/s)", color='tab:red')
    ax2.plot(wind_data['timestamp'], wind_data['wind_speed_mps'], color='tab:red', linestyle='--', label='Speed')
    ax2.tick_params(axis='y', labelcolor='tab:red')

    fig.tight_layout()
    plt.title("Wind Direction and Speed (Past 24 Hours)")
    plt.savefig(output_path)
    plt.close()
